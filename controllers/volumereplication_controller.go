...
func (r *VolumeReplicationReconciler) Reconcile(ctx context.Context, req ctrl.Request) (ctrl.Result, error) {
	...
	targetVolumeHandle := instance.Spec.TargetVolumeHandle
	...
	if targetVolumeHandle != "" {
		logger.Info("Target volume handle", "TargetVolumeHandleName", targetVolumeHandle)
	}
	...
	if instance.GetDeletionTimestamp().IsZero() {
		...
		if contains(instance.GetFinalizers(), volumeReplicationFinalizer) {
			err := r.disableVolumeReplication(logger, volumeHandle, targetVolumeHandle, parameters, secret)
			if err != nil {
	...
	if err = r.enableReplication(logger, volumeHandle, targetVolumeHandle, parameters, secret); err != nil {
	...
	case replicationv1alpha1.Primary:
		replicationErr = r.markVolumeAsPrimary(instance, logger, volumeHandle, targetVolumeHandle, parameters, secret)
	...
	case replicationv1alpha1.Secondary:
		...
		if instance.Status.State != replicationv1alpha1.SecondaryState {
			replicationErr = r.markVolumeAsSecondary(instance, logger, volumeHandle, targetVolumeHandle, parameters, secret)
			...
			if replicationErr == nil {
				...
				requeueForResync, replicationErr = r.resyncVolume(instance, logger, volumeHandle, targetVolumeHandle, parameters, secret)
			}
		}
...
func (r *VolumeReplicationReconciler) markVolumeAsPrimary(volumeReplicationObject *replicationv1alpha1.VolumeReplication, 
		logger logr.Logger, volumeID string, targetVolumeId string, parameters, secrets map[string]string) error {
...
func (r *VolumeReplicationReconciler) resyncVolume(volumeReplicationObject *replicationv1alpha1.VolumeReplication,
	logger logr.Logger, volumeID string, targetVolumeId string, parameters, secrets map[string]string) (bool, error) {
...
func (r *VolumeReplicationReconciler) disableVolumeReplication(logger logr.Logger, volumeID string, targetVolumeId string,
...
func (r *VolumeReplicationReconciler) enableReplication(logger logr.Logger, volumeID string, targetVolumeId string,
		parameters, secrets map[string]string) error {
	c := replication.CommonRequestParameters{
		VolumeID:       volumeID,
		TargetVolumeId: targetVolumeId,
